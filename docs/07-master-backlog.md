...
- [x] **JS↔Rust 對拍**:同輸入跑兩份,逐手比對可觀察狀態抓漂移(`diff/`,seed 0..50 整場一致;已接 CI)。
      抓到並修掉三條漂移:投射物攔截(採納進原型)、房 1 佈局回填、tier★★ 同步。對 native Rust(==WASM)。

> **🚪 階段 B 閘門**:同種子同操作序列 bit 一致 ✅ + harness 全房可解(0–5 含 boss ✅)+ JS↔Rust 對拍 ✅。
> **閘門全達成。**
...
- [x] **行為計數器 + run 總結**:`events::Cause` → `Died` 帶歸因;殼端逐手累計、收場命名流派
      (🔥火流 / 💥推進流 / ✨魔法彈流 / 🌀混合)。火燒擊殺 / 推進危險格擊殺 / 加速取得已上;
      鏈式串殺待連鎖閃電(第二批法術)才有。對拍/smoke 仍綠。
...
- [ ] **Demo 2 蓋基地 + 守**:**MVP 已定案 → 見 `docs/08-demo2-base-editor.md`**(離線、單房 出生點→核心、
      勝利=到核心或清場、編輯器調色盤、**bot 試打**、突襲者基礎包+威脅牌、localStorage 存一座)。
      sim 小改:`C` 核心格 + base-raid 終局。**前置 = Demo 0/1 真人閘門過關才重度開工。**
